...
- [x] **FR-1.1**: The framework shall support the following target platforms: Linux, baremetal ARM, FreeRTOS ARM.

- [x] **FR-1.2**: The framework shall provide a platform-specific `main()` entry point that dispatches to an
...
- [x] **FR-1.3**: On non-Linux targets, the framework shall provide a minimal C runtime adaptation layer to support
...
- [x] **FR-1.4**: On RTOS targets, the framework shall provide the RTOS integration necessary to run `appMain()` as the
      first user thread.
...
- [x] **FR-1.5**: On Linux, the framework shall optionally provide compile-time access to standard filesystem paths for
...
- [x] **FR-2.1**: The framework shall expose compile-time constants describing the build configuration: the compiler
      identity, compiler version (major, minor, and patch), and build type.

- [x] **FR-2.2**: The framework shall expose git repository metadata as runtime string values, captured at CMake
      configuration time:
...
- [x] **FR-2.3**: The framework shall provide a function to print the git tag to standard output, and a function to
      print a formatted build summary including compiler, build type, git metadata, and build timestamp.

### 3. Toolchain

- [x] **FR-3.1**: The framework shall configure the compiler, linker, and associated tools for the target platform,
...
- [x] **FR-3.2**: For Linux targets, the framework shall support GCC and Clang compiler variants for both native
      compilation and cross-compilation to ARM64 targets.

- [x] **FR-3.3**: For non-Linux (ARM) targets, the framework shall support the appropriate cross-compilation toolchain.
      The path to the compiler shall be configurable.

- [x] **FR-3.4**: For ARM64 Linux cross-compilation, the path to the compiler shall be configurable.

- [x] **FR-3.5**: For Linux targets, the framework shall support enabling runtime sanitizers: address, leak, thread, and
      undefined behavior. Only one sanitizer may be enabled per build. Additional sanitizer behavior shall be
      configurable.

- [x] **FR-3.6**: For Linux targets, the framework shall support enabling code coverage instrumentation.

- [x] **FR-3.7**: The framework shall integrate with the OSAL library and automatically communicate the appropriate OSAL
      backend for the active platform, without requiring the user to specify it separately.

- [x] **FR-3.8**: For baremetal ARM and FreeRTOS ARM targets, the framework shall provide a CMake helper function to
      produce raw binary images from ELF outputs.

### 4. CMake Integration

- [x] **FR-4.1**: The framework shall be integrable into dependent projects via `find_package(platform COMPONENTS ...)`.
