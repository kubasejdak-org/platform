option(USE_ASAN "platform: Use address sanitizer" OFF)
option(USE_LSAN "platform: Use leak sanitizer" OFF)
option(USE_TSAN "platform: Use thread sanitizer" OFF)
option(USE_UBSAN "platform: Use undefined behavior sanitizer" OFF)
option(SANITIZER_DISABLE_FORTIFY "platform: Disable _SOURCE_FORTIFY definition" OFF)
option(SANITIZER_DISABLE_OPTIMIZATION "platform: Disable optimizations" OFF)
...
function (enable_sanitizers)
    ...
    if (SANITIZER_FLAG_COUNT GREATER 1)
        message(FATAL_ERROR "platform: Cannot use more than one sanitizer at once")
    endif ()

    if (USE_ASAN)
        message(STATUS "platform: Enabling address sanitizer")
        set(ASAN_FLAGS          "-fsanitize=address -fno-omit-frame-pointer -fsanitize-address-use-after-scope")
    endif ()
    if (USE_LSAN)
        message(STATUS "platform: Enabling leak sanitizer")
        set(LSAN_FLAGS          "-fsanitize=leak -fno-omit-frame-pointer")
    endif ()
    if (USE_TSAN)
        message(STATUS "platform: Enabling thread sanitizer")
        set(TSAN_FLAGS         "-fsanitize=thread")
    endif ()
    if (USE_UBSAN)
        message(STATUS "platform: Enabling undefined behavior sanitizer")
